...
export class CommentsService {
  ...
  async createForBurger(input: CreateCommentInput): Promise<CommentDto> {
    ...
    async findByBurgerPaginated(burgerId: string, page = 1, limit = 10): Promise<PaginatedResponseDto<CommentDto>> {
    const safePage = Number.isFinite(page) && page > 0 ? page : 1;
    const safeLimit = Number.isFinite(limit) && limit > 0 ? Math.min(limit, 50) : 10;

    const [data, total] = await Promise.all([
      this.commentsRepo.findByBurgerPaginated({ burgerId, page: safePage, limit: safeLimit }),
      this.commentsRepo.countByBurger(burgerId),
    ]);

    return {
      data: data.map(CommentsMapper.toDto),
      total,
      page: safePage,
      limit: safeLimit,
    };
